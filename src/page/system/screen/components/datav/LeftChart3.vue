<template>
  <div class="left-chart-1">
    <dv-border-box-7>
      <div class="lc1-header">平均处理个数排名</div>
      <div class="lc1-details">总均值<span>{{average}} 个</span></div>
    </dv-border-box-7>
...
export default {
 ...
 //this.config.data=[{"name":"gpu01","value":500965.62},{"name":"gpu01","value":500965.62}]
     const { data: result }= await this.$http.get("charts/main/num")
        console.log("左三刷新");
         var  res = result
        ...
        avg=avg/res.length
         this.average=parseFloat(avg).toFixed(0)
