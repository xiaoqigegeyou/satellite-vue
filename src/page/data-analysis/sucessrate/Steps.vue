  ...
  methods: {
    ...
    async getStepsData() {
      ...
      for (let item of this.nodeList) {
        ...
        const { data: result } = await this.$http.get("successRate/steps", {
          params,
        });
    ...
    generatePerStepReport() {
      ...
      for (let index = 0; index < len; index++) {
        ...
        const ave = parseFloat(
          (
            step_data.reduce((a, b) => parseFloat(a) + parseFloat(b)) /
            step_data.length
          ).toFixed(2)
        );
      ...
      <br/>`;
        ...
        ht += `<div>所有步骤的平均成功率为：<span style="color:red"><b>${ave} %</b></span></div>
      <br/>
      <div>成功率最高的步骤为<span style="color:green"><b>${max.step_name}</b></span>
      ：<span style="color:green"><b>${max.amount} %</b></span></div>
      <br/>
      <div>成功率最低的步骤为<span style="color:red"><b>${min.step_name}</b></span>
      ：<span style="color:red"><b>${min.amount} %</b></span></div>
      <br/>
      <div>可能需要注意的步骤如下（成功率低于90%）：</div>
      `;
        abnormal_data.forEach((item) => {
          ht += `<br/><div><span style="color:red"><b>${item.step_name}:${item.amount}%</b></span></div>`;
        });
        ht += "--------------------------------------------------------------";
      }
