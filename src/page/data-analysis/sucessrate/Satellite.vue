  ...
  methods: {
    ...
    async getSatelliteData() {
      ...
      for (let item of this.nodeList) {
        ...
        const { data: result } = await this.$http.get("successRate/satellite", {
          params,
        });
    ...
    generatePerSatelliteReport() {
      ...
      for (let index = 0; index < len; index++) {
        ...
        ht += `<div>平均成功率为：<span style="color:red"><b>${ave} %</b></span></div>
          <br/>
          <div>成功率最高的卫星为<span style="color:green"><b>${max.sate_name}</b></span>
          : <span style="color:green"><b>${max.amount} %</b></span></div>
          <br/>
          <div>成功率最低的卫星为<span style="color:red"><b>${min.sate_name}</b></span>
          : <span style="color:red"><b>${min.amount} %</b></span></div>
          <br/>
          <div>可能需要注意的卫星如下（成功率低于90%）：</div>
          `;
        abnormal_data.forEach((item) => {
          ht += `<br/><div><span style="color:red"><b>${item.sate_name}:${item.amount}%</b></span></div>`;
        });
        ht += "--------------------------------------------------------------";
      }
    ...
    generatePeport() {
      ...
      for (let index = 0; index < len; index++) {
        ...
        ht += `<div>平均成功率为：<span style="color:red"><b>${ave} %</b></span></div>
          <br/>
          <div>成功率最高的日期为<span style="color:green"><b>${max.sate_name}</b></span>
          : <span style="color:green"><b>${max.amount} %</b></span></div>
          <br/>
          <div>成功率最低的日期为<span style="color:red"><b>${min.sate_name}</b></span>
          : <span style="color:red"><b>${min.amount} %</b></span></div>
          <br/>
