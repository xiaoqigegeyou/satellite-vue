  ...
  methods: {
    ...
    async getProvinceData() {
      ...
      for (let item of this.nodeList) {
        ...
        const { data: result } = await this.$http.get("processTime/province", {
          params,
        });
    ...
    generatePerProvinceReport() {
      ...
      for (let index = 0; index < len; index++) {
        ...
        const ave = parseFloat(
          (
            province_data.reduce((a, b) => a + b) / province_data.length
          ).toFixed(1)
        );
        ...
        ht += `<div>所有省份的平均处理时间为：<span style="color:red"><b>${ave} s</b></span></div>
          <br/>
          <div>处理耗时最短的省份为<span style="color:green"><b>${min.province}</b></span>,
          耗时<span style="color:green"><b>${min.amount} s</b></span></div>
          <br/>
          <div>处理耗时最长的省份为<span style="color:red"><b>${max.province}</b></span>,
          耗时<span style="color:red"><b>${max.amount} s</b></span></div>
          <br/>
          <div>可能需要注意的省份如下（处理时间超多均值的80%）：</div>
          `;
        abnormal_data.forEach((item) => {
          ht += `<br/><div><span style="color:red"><b>${item.province}:${item.amount}s</b></span></div>`;
        });
        ht += "--------------------------------------------------------------";
      }
